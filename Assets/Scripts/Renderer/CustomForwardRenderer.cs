...
public class CustomForwardRenderer : ScriptableRenderer
{
    ...
    CustomTransparentSettingsPass m_TransparentSettingsPass;
    DrawObjectsPass m_RenderTransparentForwardPass;
    CustomInvokeOnRenderObjectCallbackPass m_OnRenderObjectCallbackPass;
    PostProcessPass m_PostProcessPass;
    PostProcessPass m_FinalPostProcessPass;
    FinalBlitPass m_FinalBlitPass;
    CustomCapturePass m_CapturePass;
    ...
    public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        //        createDepthTexture |= (renderingData.cameraData.renderType == CameraRenderType.Base
        //                               && !renderingData.resolveFinalTarget);
        createDepthTexture |= (renderingData.cameraData.renderType == CameraRenderType.Base
                                && !IsResolveFinalTarget(camera));
        ...
        if (cameraData.renderType == CameraRenderType.Base)
        {
            m_ActiveCameraColorAttachment =
                (createColorTexture) ? m_CameraColorAttachment : RenderTargetHandle.CameraTarget;
            m_ActiveCameraDepthAttachment =
                (createDepthTexture) ? m_CameraDepthAttachment : RenderTargetHandle.CameraTarget;
...
#pragma warning restore 0618
        ...
        bool lastCameraInTheStack = IsResolveFinalTarget(camera);
        ...
        bool dontResolvePostProcessingToCameraTarget =
...

        {
            if (lastCameraInTheStack)
            {
                ...
                if (applyPostProcessing)
                {
                    var destination = dontResolvePostProcessingToCameraTarget
                        ? m_AfterPostProcessColor
                        : RenderTargetHandle.CameraTarget;

                    // if resolving to screen we need to be able to perform sRGBConvertion in post-processing if necessary
                    bool doSRGBConvertion = !(dontResolvePostProcessingToCameraTarget ||
                                              (m_ActiveCameraColorAttachment != RenderTargetHandle.CameraTarget));
                    m_PostProcessPass.Setup(cameraTargetDescriptor, m_ActiveCameraColorAttachment, destination,
                        m_ActiveCameraDepthAttachment, m_ColorGradingLut, applyFinalPostProcessing, doSRGBConvertion);
                ...
                var sourceForFinalPass =
                    (applyPostProcessing) ? m_AfterPostProcessColor : m_ActiveCameraColorAttachment;
    ...
    void CreateCameraRenderTarget(ScriptableRenderContext context, ref CameraData cameraData)
    {
        ...
        if (m_ActiveCameraDepthAttachment != RenderTargetHandle.CameraTarget)
        {
            ...
            depthDescriptor.bindMS = msaaSamples > 1 && !SystemInfo.supportsMultisampleAutoResolve &&
    ...
    bool RequiresIntermediateColorTexture(ref RenderingData renderingData, RenderTextureDescriptor baseDescriptor)
    {
        ...
        if (renderingData.cameraData.renderType == CameraRenderType.Base && !IsResolveFinalTarget(renderingData.cameraData.camera))
            return true;
 ...
 UnityEngine.XR.XRSettings.deviceEyeTextureDimension == baseDescriptor.dimension;
#endif
        ...
        return requiresBlitForOffscreenCamera || cameraData.isSceneViewCamera || isScaledRender ||
    ...
    bool IsResolveFinalTarget(Camera camera)
    {
        return true;
    }
}